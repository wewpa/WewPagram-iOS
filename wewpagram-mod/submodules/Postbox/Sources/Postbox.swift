...
public final class Transaction {
    ...
    // Read-only resolver, added for WewPagram's deleted-message archive feature.
    // Does not delete or modify anything — just exposes the existing private
    // lookup so callers can read a message's content before it gets deleted.
    public func wewResolveGlobalMessageIds(_ ids: [Int32]) -> [MessageId] {
        assert(!self.disposed)
        return self.postbox?.messageIdsForGlobalIds(ids) ?? []
    }
