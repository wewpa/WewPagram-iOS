...
public class ChatMessageDateAndStatusNode: ASDisplayNode {
    ...
    public struct Arguments {
        ...
        public init(
            ...
            self.wewIsDeletedGhost = wewIsDeletedGhost
        }
    }
    ...
    private var wewDeletedIcon: ASImageNode?
            ...
            return (resultingWidth, { boundingWidth in
                return (CGSize(width: boundingWidth, height: resultingHeight), { animation in
                    if let strongSelf = self {
                        ...
                        if arguments.wewIsDeletedGhost {
                            let iconSize = CGSize(width: 10.0, height: 10.0)
                            let iconNode: ASImageNode
                            if let existing = strongSelf.wewDeletedIcon {
                                iconNode = existing
                            } else {
                                iconNode = ASImageNode()
                                iconNode.isLayerBacked = true
                                iconNode.displaysAsynchronously = false
                                iconNode.image = WewDeletedGhostIcon.image
                                strongSelf.wewDeletedIcon = iconNode
                            ...
                            existing.removeFromSupernode()
                            strongSelf.wewDeletedIcon = nil
                        }
