...
func infoItems(
    ...
    if case let .user(user) = data.peer {
        ...
        if let phone = user.phone {
            let wewFakePhone = isMyProfile ? WewPagramSettings.shared.fakePhoneNumber : nil
            let formattedPhone = wewFakePhone ?? formatPhoneNumber(context: context, number: phone)
            let label: String
        ...
        if let mainUsername = user.addressName {
            ...
            var usernameParts = user.usernames.filter { $0.isActive && $0.username != mainUsername }.map { "@\($0.username)" }
            if isMyProfile {
                usernameParts.append(contentsOf: WewPagramSettings.shared.fakeNftEntries.map { "@\($0.username)" })
            }
            if !usernameParts.isEmpty {
                additionalUsernames = presentationData.strings.Profile_AdditionalUsernames(usernameParts.joined(separator: ", ")).string
            }
