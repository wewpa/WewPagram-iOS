...
private func isActivityMutedByGhostMode(_ activity: PeerInputActivity) -> Bool {
    let s = WewPagramSettings.shared
    switch activity {
    case .typingText:
        return s.disableTyping
    case .recordingVoice:
        return s.disableVoiceRecording
    case .uploadingFile:
        // File uploads cover voice-note uploads on the wire; treat them under
        // the voice-message upload switch, which is the closest user-visible name.
        return s.disableVoiceUploading
    case .uploadingPhoto:
        return s.disableUploadingPhoto
    case .uploadingVideo:
        return s.disableUploadingVideo
    case .recordingInstantVideo:
        return s.disableRecordingVideo
    case .uploadingInstantVideo:
        return s.disableUploadingVideo
    case .playingGame,
         .speakingInGroupCall,
         .choosingSticker,
         .interactingWithEmoji,
         .seeingEmojiInteraction:
        return false
    }
}
...
private func requestActivity(postbox: Postbox, network: Network, accountPeerId: PeerId, peerId: PeerId, threadId: Int64?, activity: PeerInputActivity?) -> Signal<Void, NoError> {
    // Selective ghost-mode filtering per activity type. A muted activity is
    // dropped entirely — the server never learns we started or stopped it.
    if let activity = activity, isActivityMutedByGhostMode(activity) {
        return .complete()
    }
